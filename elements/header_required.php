...
$metaTags['generator'] = sprintf('<meta name="generator" content="%s"/>', 'PortoPackage (www.cplusplus-development.de)');
...
echo "<title>".htmlspecialchars($pageTitle, ENT_COMPAT, APP_CHARSET)."</title>";
...
echo '<script> var CCM_DISPATCHER_FILENAME="'. DIR_REL . '/' . DISPATCHER_FILENAME.'", CCM_CID="'.(($cID)?$cID:0).'", CCM_EDIT_MODE='.(($isEditMode)?'true':'false').',CCM_ARRANGE_MODE='.(($isArrangeMode)?'true':'false').',CCM_IMAGE_PATH="'. ASSETS_URL_IMAGES.'",CCM_TOOLS_PATH="'.REL_DIR_FILES_TOOLS_REQUIRED.'",CCM_APPLICATION_URL="'. \Core::getApplicationURL().'",CCM_REL="'. \Core::getApplicationRelativePath().'"; </script>';
...
if (is_object($cp))
{
    View::element('page_controls_header', array('cp' => $cp, 'c' => $c), 'porto');
...
{
    echo '<style type="text/css">'. $scc->getValue(). '</style>';
}
